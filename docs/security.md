...
- Default Compute and App Engine service accounts are retained, while exact negative IAM resources remove only their automatic Editor grants; a project policy blocks future automatic basic-role grants, and the Compute identity keeps the explicit build role required by the Functions deployment path.
